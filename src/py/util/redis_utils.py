...
from constants import (
	GARAGE_BINARY, REDIS_CLI_BINARY, REDIS_WORKER_NETWORK_DB, UFW,
)
...
def extend_network(
	previous_instance_ips: Iterable[str], workers: Iterable[str],
):
	threads: List[Popen] = []
	for previous_ip in previous_instance_ips:
		workers = set(workers) - {previous_ip}
		if workers:
			...
			threads.append(ssh_do(previous_ip, (
				f"{UFW} allow from {new_ip}" for new_ip in previous_ip
			)))
